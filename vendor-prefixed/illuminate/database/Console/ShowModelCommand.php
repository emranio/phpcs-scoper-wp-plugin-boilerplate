...
namespace MyVendorPrefix\Illuminate\Database\Console;

use BackedEnum;
use MyVendorPrefix\Doctrine\DBAL\Schema\Column;
use MyVendorPrefix\Doctrine\DBAL\Schema\Index;
use MyVendorPrefix\Doctrine\DBAL\Types\DecimalType;
use MyVendorPrefix\Illuminate\Contracts\Container\BindingResolutionException;
use MyVendorPrefix\Illuminate\Database\Eloquent\Model;
use MyVendorPrefix\Illuminate\Database\Eloquent\Relations\Relation;
use MyVendorPrefix\Illuminate\Support\Facades\Gate;
use MyVendorPrefix\Illuminate\Support\Str;
use ReflectionClass;
use ReflectionMethod;
use SplFileObject;
use MyVendorPrefix\Symfony\Component\Console\Attribute\AsCommand;
...
class ShowModelCommand extends DatabaseInspectionCommand
{
    ...
    public function handle()
    {
        ...
        $this->display($class, $model->getConnection()->getName(), $model->getConnection()->getTablePrefix() . $model->getTable(), $this->getPolicy($model), $this->getAttributes($model), $this->getRelations($model), $this->getObservers($model));
    }
    /**
    ...
    protected function getVirtualAttributes($model, $columns)
    {
        ...
        return collect($class->getMethods())->reject(fn(ReflectionMethod $method) => $method->isStatic() || $method->isAbstract() || $method->getDeclaringClass()->getName() === Model::class)->mapWithKeys(function (ReflectionMethod $method) use ($model) {
        ...
        })->reject(fn($cast, $name) => collect($columns)->has($name))->map(fn($cast, $name) => ['name' => $name, 'type' => null, 'increments' => \false, 'nullable' => null, 'default' => null, 'unique' => null, 'fillable' => $model->isFillable($name), 'hidden' => $this->attributeIsHidden($name, $model), 'appended' => $model->hasAppended($name), 'cast' => $cast])->values();
    }
    /**
    ...
    {
        return collect(get_class_methods($model))->map(fn($method) => new ReflectionMethod($model, $method))->reject(fn(ReflectionMethod $method) => $method->isStatic() || $method->isAbstract() || $method->getDeclaringClass()->getName() === Model::class)->filter(function (ReflectionMethod $method) {
            ...
            return collect($this->relationMethods)->contains(fn($relationMethod) => str_contains($code, '$this->' . $relationMethod . '('));
    ...
    {
        $this->option('json') ? $this->displayJson($class, $database, $table, $policy, $attributes, $relations, $observers) : $this->displayCli($class, $database, $table, $policy, $attributes, $relations, $observers);
    }
    /**
    ...
    {
        $this->output->writeln(collect(['class' => $class, 'database' => $database, 'table' => $table, 'policy' => $policy, 'attributes' => $attributes, 'relations' => $relations, 'observers' => $observers])->toJson());
    }
    /**
    ...
    protected function displayCli($class, $database, $table, $policy, $attributes, $relations, $observers)
    {
        ...
        foreach ($attributes as $attribute) {
            $first = trim(sprintf('%s %s', $attribute['name'], collect(['increments', 'unique', 'nullable', 'fillable', 'hidden', 'appended'])->filter(fn($property) => $attribute[$property])->map(fn($property) => sprintf('<fg=gray>%s</>', $property))->implode('<fg=gray>,</> ')));
            $second = collect([$attribute['type'], $attribute['cast'] ? '<fg=yellow;options=bold>' . $attribute['cast'] . '</>' : null])->filter()->implode(' <fg=gray>/</> ');
    ...
    {
        return collect($model->getDates())->filter()->flip()->map(fn() => 'datetime')->merge($model->getCasts());
    }
    /**
    ...
    {
        return collect($indexes)->filter(fn(Index $index) => count($index->getColumns()) === 1 && $index->getColumns()[0] === $column)->contains(fn(Index $index) => $index->isUnique());
    }
    /**
