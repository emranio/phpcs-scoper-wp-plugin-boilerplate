...
namespace MyVendorPrefix\Illuminate\Database\Schema\Grammars;

use MyVendorPrefix\Doctrine\DBAL\Schema\AbstractSchemaManager as SchemaManager;
...
use MyVendorPrefix\Illuminate\Database\Schema\Blueprint;
use MyVendorPrefix\Illuminate\Support\Fluent;
...
class ChangeColumn
{
    ...
    protected static function getChangedDiff($grammar, Blueprint $blueprint, SchemaManager $schema)
    {
        ...
        return $schema->createComparator()->compareTables($current, static::getTableWithColumnChanges($blueprint, $current));
    }
    /**
    ...
    {
        return $table->modifyColumn($fluent['name'], static::getDoctrineColumnChangeOptions($fluent))->getColumn($fluent['name']);
    }
    /**
