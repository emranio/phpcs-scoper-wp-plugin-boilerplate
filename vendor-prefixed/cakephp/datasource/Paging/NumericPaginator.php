 ...
 */
namespace MyVendorPrefix\Cake\Datasource\Paging;

use MyVendorPrefix\Cake\Core\Exception\CakeException;
use MyVendorPrefix\Cake\Core\InstanceConfigTrait;
use MyVendorPrefix\Cake\Datasource\Paging\Exception\PageOutOfBoundsException;
use MyVendorPrefix\Cake\Datasource\QueryInterface;
use MyVendorPrefix\Cake\Datasource\RepositoryInterface;
use MyVendorPrefix\Cake\Datasource\ResultSetInterface;
...
class NumericPaginator implements PaginatorInterface
{
    ...
    public function paginate(object $object, array $params = [], array $settings = []): ResultSetInterface
    {
        ...
        if ($pagingParams['requestedPage'] > $pagingParams['page']) {
            throw new PageOutOfBoundsException(['requestedPage' => $pagingParams['requestedPage'], 'pagingParams' => $this->_pagingParams]);
        }
        return $results;
    }
    /**
    ...
    protected function buildParams(array $data): array
    {
        ...
        $paging = ['count' => $data['count'], 'current' => $data['numResults'], 'perPage' => $limit, 'page' => $data['options']['page'], 'requestedPage' => $data['options']['page']];
        ...
        $paging += ['limit' => $data['defaults']['limit'] != $limit ? $limit : null, 'scope' => $data['options']['scope'], 'finder' => $data['finder']];
        return $paging;
    }
    /**
    ...
    protected function addSortingParams(array $params, array $data): array
    {
        ...
        $params += ['sort' => $data['options']['sort'], 'direction' => isset($data['options']['sort']) && count($order) ? current($order) : null, 'sortDefault' => $sortDefault, 'directionDefault' => $directionDefault, 'completeSort' => $order];
        return $params;
    }
    /**
    ...
    public function validateSort(RepositoryInterface $object, array $options): array
    {
        ...
        if ($allowed !== null) {
            ...
            $sortAllowed = in_array($field, $allowed, \true);
            if (!$sortAllowed) {
                $options['order'] = [];
                $options['sort'] = null;
                return $options;
...
// phpcs:disable
class_alias('MyVendorPrefix\Cake\Datasource\Paging\NumericPaginator', 'MyVendorPrefix\Cake\Datasource\Paginator');
// phpcs:enable
