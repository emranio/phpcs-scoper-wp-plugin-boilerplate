...
use MyVendorPrefix\Cake\Collection\Collection;
use MyVendorPrefix\Cake\ORM\Entity;
use MyVendorPrefix\Cake\Utility\Hash;
use MyVendorPrefix\Cake\Utility\Inflector;
...
trait EntityTrait
{
    ...
    public function set($field, $value = null, array $options = [])
    {
        ...
        $options += ['setter' => \true, 'guard' => $guard];
        foreach ($field as $name => $value) {
            $name = (string) $name;
            if ($options['guard'] === \true && !$this->isAccessible($name)) {
                continue;
            }
            $this->setDirty($name, \true);
            if (!array_key_exists($name, $this->_original) && array_key_exists($name, $this->_fields) && $this->_fields[$name] !== $value) {
    ...
    public function getOriginalValues(): array
    {
        ...
        foreach ($this->_fields as $key => $value) {
            if (!in_array($key, $originalKeys, \true)) {
                $originals[$key] = $value;
            }
        }
        return $originals;
    ...
    public function isEmpty(string $field): bool
    {
        ...
        if ($value === null || (is_array($value) && empty($value) || is_string($value) && $value === '')) {
            return \true;
        }
        return \false;
    }
    ...
    protected function _nestedErrors(string $field): array
    {
        ...
        while ($len) {
            ...
            if (is_array($val) || $val instanceof Traversable || $val instanceof EntityInterface) {
                $entity = $val;
            } else {
                $path[] = $part;
                break;
    ...
    public function __debugInfo(): array
    {
        ...
        return $fields + ['[new]' => $this->isNew(), '[accessible]' => $this->_accessible, '[dirty]' => $this->_dirty, '[original]' => $this->_original, '[virtual]' => $this->_virtual, '[hasErrors]' => $this->hasErrors(), '[errors]' => $this->_errors, '[invalid]' => $this->_invalid, '[repository]' => $this->_registryAlias];
    }
}
