 ...
 */
namespace MyVendorPrefix\Cake\Database\Statement;
...
class PDOStatement extends StatementDecorator
{
    ...
    public function fetch($type = parent::FETCH_TYPE_NUM)
    {
        ...
        if (!is_int($type)) {
            throw new CakeException(sprintf('Fetch type for PDOStatement must be an integer, found `%s` instead', getTypeName($type)));
        }
