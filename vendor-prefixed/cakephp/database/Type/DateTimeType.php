 ...
 */
namespace MyVendorPrefix\Cake\Database\Type;

use MyVendorPrefix\Cake\Chronos\ChronosDate;
use MyVendorPrefix\Cake\Database\DriverInterface;
use MyVendorPrefix\Cake\I18n\FrozenTime;
use MyVendorPrefix\Cake\I18n\I18nDateTimeInterface;
use MyVendorPrefix\Cake\I18n\Time;
use DateTime;
...
class DateTimeType extends BaseType implements BatchCastingInterface
{
     ...
     * @var array<string>
     */
    protected $_marshalFormats = ['Y-m-d H:i', 'Y-m-d H:i:s', 'Y-m-d\TH:i', 'Y-m-d\TH:i:s', 'Y-m-d\TH:i:sP'];
    /**
    ...
    public function toDatabase($value, DriverInterface $driver): ?string
    {
        ...
        if ($this->dbTimezone !== null && $this->dbTimezone->getName() !== $value->getTimezone()->getName()) {
    ...
    public function toPHP($value, DriverInterface $driver)
    {
        ...
        if (!$this->keepDatabaseTimezone && $instance->getTimezone()->getName() !== $this->defaultTimezone->getName()) {
    ...
    public function marshal($value): ?DateTimeInterface
    {
        ...
        $format = '';
        if (isset($value['year'], $value['month'], $value['day']) && (is_numeric($value['year']) && is_numeric($value['month']) && is_numeric($value['day']))) {
        ...
        $format .= sprintf('%s%02d:%02d:%02d.%06d', empty($format) ? '' : ' ', $value['hour'], $value['minute'], $value['second'], $value['microsecond']);
        /** @var \DateTime|\DateTimeImmutable $dateTime */
    ...
    public function useLocaleParser(bool $enable = \true)
    {
        ...
        throw new RuntimeException(sprintf('Cannot use locale parsing with the %s class', $this->_className));
    }
    /**
    ...
    {
        deprecationWarning('Configuring immutable or mutable classes is deprecated and immutable' . ' classes will be the permanent configuration in 5.0. Calling `useImmutable()` is unnecessary.');
        $this->_setClassName(FrozenTime::class, DateTimeImmutable::class);
        return $this;
    }
    /**
