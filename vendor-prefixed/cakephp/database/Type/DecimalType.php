...
use MyVendorPrefix\Cake\I18n\Number;
...
class DecimalType extends BaseType implements BatchCastingInterface
{
    ...
    public function toDatabase($value, DriverInterface $driver)
    {
        ...
        if (is_object($value) && method_exists($value, '__toString') && is_numeric(strval($value))) {
            return strval($value);
        }
        throw new InvalidArgumentException(sprintf('Cannot convert value of type `%s` to a decimal', getTypeName($value)));
    }
    /**
     * {@inheritDoc}
     *
    ...
    public function useLocaleParser(bool $enable = \true)
    {
        ...
        throw new RuntimeException(sprintf('Cannot use locale parsing with the %s class', static::$numberClass));
    }
    /**
