...
use MyVendorPrefix\Cake\Database\Retry\ErrorCodeWaitStrategy;
use MyVendorPrefix\Cake\Database\Schema\SchemaDialect;
use MyVendorPrefix\Cake\Database\Schema\TableSchema;
...
abstract class Driver implements DriverInterface
{
    ...
    protected function _connect(string $dsn, array $config): bool
    {
        $action = function () use ($dsn, $config) {
            $this->setConnection(new PDO($dsn, $config['username'] ?: null, $config['password'] ?: null, $config['flags']));
        };
        $retry = new CommandRetry(new ErrorCodeWaitStrategy(static::RETRY_ERROR_CODES, 5), 4);
        try {
            $retry->run($action);
        } catch (PDOException $e) {
            throw new MissingConnectionException(['driver' => App::shortName(static::class, 'Database/Driver'), 'reason' => $e->getMessage()], null, $e);
        } finally {
    ...
    public function getConnection()
    {
        if ($this->_connection === null) {
            throw new MissingConnectionException(['driver' => App::shortName(static::class, 'Database/Driver'), 'reason' => 'Unknown']);
        }
    ...
    public function schemaValue($value): string
    {
        ...
        if (is_int($value) || $value === '0' || is_numeric($value) && strpos($value, ',') === \false && substr($value, 0, 1) !== '0' && strpos($value, 'e') === \false) {
            return (string) $value;
        }
     ...
     * @inheritDoc
     */
    public function enableAutoQuoting(bool $enable = \true)
    {
