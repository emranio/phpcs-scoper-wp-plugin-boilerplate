...
use MyVendorPrefix\Cake\Database\TypedResultInterface;
use MyVendorPrefix\Cake\Database\TypeMapTrait;
...
class CaseStatementExpression implements ExpressionInterface, TypedResultInterface
{
     ...
     * @var array<string>
     */
    protected $validClauseNames = ['value', 'when', 'else'];
    /**
    ...
    {
        if (func_num_args() > 0) {
            if ($value !== null && !is_scalar($value) && !(is_object($value) && !$value instanceof Closure)) {
                throw new InvalidArgumentException(sprintf('The `$value` argument must be either `null`, a scalar value, an object, ' . 'or an instance of `\%s`, `%s` given.', ExpressionInterface::class, getTypeName($value)));
            }
            $this->value = $value;
    ...
    public function when($when, $type = null)
    {
        ...
        if ($when instanceof Closure) {
            ...
            if (!$when instanceof WhenThenExpression) {
                throw new LogicException(sprintf('`when()` callables must return an instance of `\%s`, `%s` given.', WhenThenExpression::class, getTypeName($when)));
            }
        }
        if ($when instanceof WhenThenExpression) {
            $this->when[] = $when;
