...
class QueryExpression implements ExpressionInterface, Countable
{
    ...
    {
        $or = new static([], [], 'OR');
        $or->notIn($field, $values, $type)->isNull($field);
        return $this->add($or);
    }
    /**
    ...
    protected function _parseCondition(string $field, $value)
    {
        ...
        if ($operator === 'is' && $value === null) {
            return new UnaryExpression('IS NULL', new IdentifierExpression($expression), UnaryExpression::POSTFIX);
        }
        if ($operator === 'is not' && $value === null) {
            return new UnaryExpression('IS NOT NULL', new IdentifierExpression($expression), UnaryExpression::POSTFIX);
        }
        if ($operator === 'is' && $value !== null) {
            $operator = '=';
        }
        ...
        if ($value === null && $this->_conjunction !== ',') {
            throw new InvalidArgumentException(sprintf('Expression `%s` is missing operator (IS, IS NOT) with `null` value.', $expression));
        }
