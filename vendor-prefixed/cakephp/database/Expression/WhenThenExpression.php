 ...
 */
namespace MyVendorPrefix\Cake\Database\Expression;

use MyVendorPrefix\Cake\Database\ExpressionInterface;
use MyVendorPrefix\Cake\Database\Query;
use MyVendorPrefix\Cake\Database\Type\ExpressionTypeCasterTrait;
use MyVendorPrefix\Cake\Database\TypeMap;
use MyVendorPrefix\Cake\Database\ValueBinder;
use Closure;
use InvalidArgumentException;
use LogicException;
use function MyVendorPrefix\Cake\Core\getTypeName;
/**
...
class WhenThenExpression implements ExpressionInterface
{
     ...
     * @var array<string>
     */
    protected $validClauseNames = ['when', 'then'];
    /**
    ...
    {
        if (!(is_array($when) && !empty($when)) && !is_scalar($when) && !is_object($when)) {
            throw new InvalidArgumentException(sprintf('The `$when` argument must be either a non-empty array, a scalar value, an object, ' . 'or an instance of `\%s`, `%s` given.', ExpressionInterface::class, is_array($when) ? '[]' : getTypeName($when)));
        }
        if ($type !== null && !is_array($type) && !is_string($type)) {
            throw new InvalidArgumentException(sprintf('The `$type` argument must be either an array, a string, or `null`, `%s` given.', getTypeName($type)));
        }
        if (is_array($when)) {
            if ($type !== null && !is_array($type)) {
                throw new InvalidArgumentException(sprintf('When using an array for the `$when` argument, the `$type` argument must be an ' . 'array too, `%s` given.', getTypeName($type)));
            }
            ...
            if ($type !== null && !is_string($type)) {
                throw new InvalidArgumentException(sprintf('When using a non-array value for the `$when` argument, the `$type` argument must ' . 'be a string, `%s` given.', getTypeName($type)));
            }
    ...
    public function then($result, ?string $type = null)
    {
        if ($result !== null && !is_scalar($result) && !(is_object($result) && !$result instanceof Closure)) {
            throw new InvalidArgumentException(sprintf('The `$result` argument must be either `null`, a scalar value, an object, ' . 'or an instance of `\%s`, `%s` given.', ExpressionInterface::class, getTypeName($result)));
        }
        $this->then = $result;
        if ($type === null) {
    ...
    {
        if (!in_array($clause, $this->validClauseNames, \true)) {
            throw new InvalidArgumentException(sprintf('The `$clause` argument must be one of `%s`, the given value `%s` is invalid.', implode('`, `', $this->validClauseNames), $clause));
        }
        return $this->{$clause};
    }
    /**
    ...
    public function sql(ValueBinder $binder): string
    {
        ...
        $when = $this->when;
        if (is_string($this->whenType) && !$when instanceof ExpressionInterface) {
