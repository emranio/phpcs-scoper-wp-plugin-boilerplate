...
use MyVendorPrefix\Cake\Database\Expression\CommonTableExpression;
use MyVendorPrefix\Cake\Database\Expression\IdentifierExpression;
...
use MyVendorPrefix\Cake\Database\Expression\QueryExpression;
use MyVendorPrefix\Cake\Database\Expression\ValuesExpression;
use MyVendorPrefix\Cake\Database\Expression\WindowExpression;
use MyVendorPrefix\Cake\Database\Statement\CallbackStatement;
use Closure;
...
class Query implements ExpressionInterface, IteratorAggregate
{
     ...
     */
    protected $_parts = ['delete' => \true, 'update' => [], 'set' => [], 'insert' => [], 'values' => [], 'with' => [], 'select' => [], 'distinct' => \false, 'modifier' => [], 'from' => [], 'join' => [], 'where' => null, 'group' => [], 'having' => null, 'window' => [], 'order' => null, 'limit' => null, 'offset' => null, 'union' => [], 'epilog' => null];
    /**
     ...
     */
    protected $_selectParts = ['with', 'select', 'from', 'join', 'where', 'group', 'having', 'order', 'limit', 'offset', 'union', 'epilog'];
    /**
    ...
    {
        $options += ['types' => [], 'allowEmpty' => \false];
    ...
    public function whereNotInListOrNull(string $field, array $values, array $options = [])
    {
        ...
        return $this->where(['OR' => [$field . ' NOT IN' => $values, $field . ' IS' => null]], $options['types']);
    }
    /**
    ...
    protected function _conjugate(string $part, $append, $conjunction, array $types): void
    {
        ...
        if ($expression->getConjunction() === $conjunction) {
            ...
            $expression = $this->newExpr()->setConjunction($conjunction)->add([$expression, $append], $types);
        }
        $this->_parts[$part] = $expression;
        $this->_dirty();
    }
