 ...
 */
namespace MyVendorPrefix\Cake\Database\Driver;

use MyVendorPrefix\Cake\Database\Driver;
use MyVendorPrefix\Cake\Database\Expression\FunctionExpression;
use MyVendorPrefix\Cake\Database\Expression\OrderByExpression;
use MyVendorPrefix\Cake\Database\Expression\OrderClauseExpression;
use MyVendorPrefix\Cake\Database\Expression\TupleComparison;
use MyVendorPrefix\Cake\Database\Expression\UnaryExpression;
use MyVendorPrefix\Cake\Database\ExpressionInterface;
use MyVendorPrefix\Cake\Database\Query;
use MyVendorPrefix\Cake\Database\QueryCompiler;
use MyVendorPrefix\Cake\Database\Schema\SchemaDialect;
use MyVendorPrefix\Cake\Database\Schema\SqlserverSchemaDialect;
use MyVendorPrefix\Cake\Database\SqlserverCompiler;
use MyVendorPrefix\Cake\Database\Statement\SqlserverStatement;
use MyVendorPrefix\Cake\Database\StatementInterface;
...
class Sqlserver extends Driver
{
    ...
    protected $_baseConfig = [
        'host' => 'MyVendorPrefix\localhost\SQLEXPRESS',
        'username' => '',
        'password' => '',
        'database' => 'cake',
        'port' => '',
    ...
    {
        return in_array('sqlsrv', PDO::getAvailableDrivers(), \true);
    }
    /**
    ...
    protected function _pagingSubquery(Query $original, ?int $limit, ?int $offset): Query
    {
        ...
        if ($original->clause('order')) {
            ...
            $original->clause('order')->iterateParts(function ($direction, $orderBy) use ($select, $order) {
                $key = $orderBy;
                if (isset($select[$orderBy]) && $select[$orderBy] instanceof ExpressionInterface) {
        ...
        $query->select(['_cake_page_rownum_' => new UnaryExpression('ROW_NUMBER() OVER', $order)])->limit(null)->offset(null)->order([], \true);
    ...
    protected function _transformDistinct(Query $query): Query
    {
        ...
        $query->select(function ($q) use ($distinct, $order) {
            $over = $q->newExpr('ROW_NUMBER() OVER')->add('(PARTITION BY')->add($q->newExpr()->add($distinct)->setConjunction(','))->add($order)->add(')')->setConjunction(' ');
            return ['_cake_distinct_pivot_' => $over];
        })->limit(null)->offset(null)->order([], \true);
        $outer = new Query($query->getConnection());
    ...
    {
        return [FunctionExpression::class => '_transformFunctionExpression', TupleComparison::class => '_transformTupleComparison'];
    }
    /**
    ...
    protected function _transformFunctionExpression(FunctionExpression $expression): void
    {
        switch ($expression->getName()) {
            ...
            case 'DATEDIFF':
                ...
                if (!$hasDay) {
                    $expression->add(['day' => 'literal'], [], \true);
                }
                break;
            case 'CURRENT_DATE':
            ...
            case 'DATE_ADD':
                ...
                $expression->setName('DATEADD')->setConjunction(',')->iterateParts($visitor)->iterateParts($manipulator)->add([$params[2] => 'literal']);
                break;
            case 'DAYOFWEEK':
                $expression->setName('DATEPART')->setConjunction(' ')->add(['weekday, ' => 'literal'], [], \true);
                break;
            case 'SUBSTR':
                $expression->setName('SUBSTRING');
                if (count($expression) < 4) {
                    $params = [];
                    $expression->iterateParts(function ($p) use (&$params) {
                        return $params[] = $p;
