...
use MyVendorPrefix\Cake\Database\Expression\ComparisonExpression;
use MyVendorPrefix\Cake\Database\Expression\IdentifierExpression;
use MyVendorPrefix\Cake\Database\IdentifierQuoter;
...
trait SqlDialectTrait
{
    ...
    protected function _transformDistinct(Query $query): Query
    {
        if (is_array($query->clause('distinct'))) {
            $query->group($query->clause('distinct'), \true);
    ...
    protected function _removeAliasesFromConditions(Query $query): Query
    {
        if ($query->clause('join')) {
            throw new RuntimeException('Aliases are being removed from conditions for UPDATE/DELETE queries, ' . 'this can break references to joined tables.');
        }
...
// phpcs:disable
class_alias('MyVendorPrefix\Cake\Database\Driver\SqlDialectTrait', 'MyVendorPrefix\Cake\Database\SqlDialectTrait');
// phpcs:enable
