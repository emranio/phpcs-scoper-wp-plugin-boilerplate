...
use MyVendorPrefix\Cake\Database\Schema\SqliteSchemaDialect;
use MyVendorPrefix\Cake\Database\SqliteCompiler;
use MyVendorPrefix\Cake\Database\Statement\PDOStatement;
use MyVendorPrefix\Cake\Database\Statement\SqliteStatement;
...
class Sqlite extends Driver
{
     ...
     */
    protected $_baseConfig = ['persistent' => \false, 'username' => null, 'password' => null, 'database' => ':memory:', 'encoding' => 'utf8', 'mask' => 0644, 'cache' => null, 'mode' => null, 'flags' => [], 'init' => []];
    /**
     ...
     */
    protected $_dateParts = ['day' => 'd', 'hour' => 'H', 'month' => 'm', 'minute' => 'M', 'second' => 'S', 'week' => 'W', 'year' => 'Y'];
    /**
     * Mapping of feature to db server version for feature availability checks.
     *
     * @var array<string, string>
     */
    protected $featureVersions = ['cte' => '3.8.3', 'window' => '3.28.0'];
    /**
    ...
    {
        return in_array('sqlite', PDO::getAvailableDrivers(), \true);
    }
    /**
    ...
    public function supports(string $feature): bool
    {
        switch ($feature) {
            ...
            case static::FEATURE_WINDOW:
                return version_compare($this->version(), $this->featureVersions[$feature], '>=');
    ...
    protected function _transformFunctionExpression(FunctionExpression $expression): void
    {
        switch ($expression->getName()) {
            case 'CONCAT':
                ...
                break;
            case 'DATEDIFF':
                $expression->setName('ROUND')->setConjunction('-')->iterateParts(function ($p) {
            ...
            case 'NOW':
                ...
                break;
            case 'RAND':
                $expression->setName('ABS')->add(['RANDOM() % 1' => 'literal'], [], \true);
                break;
            case 'CURRENT_DATE':
            ...
            case 'CURRENT_TIME':
                ...
                break;
            case 'EXTRACT':
                $expression->setName('STRFTIME')->setConjunction(' ,')->iterateParts(function ($p, $key) {
                    if ($key === 0) {
                ...
                break;
            case 'DATE_ADD':
                $expression->setName('DATE')->setConjunction(',')->iterateParts(function ($p, $key) {
                    if ($key === 1) {
                        $p = ['value' => $p, 'type' => null];
                    }
                    return $p;
                ...
                break;
            case 'DAYOFWEEK':
                $expression->setName('STRFTIME')->setConjunction(' ')->add(["'%w', " => 'literal'], [], \true)->add([') + (1' => 'literal']);
