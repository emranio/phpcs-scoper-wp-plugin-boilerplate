 ...
 */
namespace MyVendorPrefix\Cake\Database\Log;

use MyVendorPrefix\Cake\Core\Configure;
use MyVendorPrefix\Cake\Database\Exception\DatabaseException;
use MyVendorPrefix\Cake\Database\Statement\StatementDecorator;
use Exception;
...
class LoggingStatement extends StatementDecorator
{
    ...
    public function execute(?array $params = null): bool
    {
        ...
        try {
            ...
            $this->loggedQuery->took = (int) round((microtime(\true) - $this->startTime) * 1000, 0);
        } catch (Exception $e) {
            $this->loggedQuery->error = $e;
            $this->_log();
            if (Configure::read('Error.convertStatementToDatabaseException', \false) === \true) {
                $code = $e->getCode();
                if (!is_int($code)) {
                    $code = null;
                }
                throw new DatabaseException(['message' => $e->getMessage(), 'queryString' => $this->queryString], $code, $e);
            }
            if (version_compare(\PHP_VERSION, '8.2.0', '<')) {
                deprecationWarning('4.4.12 - Having queryString set on exceptions is deprecated.' . 'If you are not using this attribute there is no action to take.' . 'Otherwise, enable Error.convertStatementToDatabaseException.');
