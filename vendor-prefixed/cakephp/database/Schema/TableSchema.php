...
use MyVendorPrefix\Cake\Database\TypeFactory;
...
class TableSchema implements TableSchemaInterface, SqlGeneratorInterface
{
     ...
     */
    public static $columnLengths = ['tiny' => self::LENGTH_TINY, 'medium' => self::LENGTH_MEDIUM, 'long' => self::LENGTH_LONG];
    /**
     ...
     * @var array<string, mixed>
     */
    protected static $_columnKeys = ['type' => null, 'baseType' => null, 'length' => null, 'precision' => null, 'null' => null, 'default' => null, 'comment' => null];
    /**
     * Additional type specific properties.
     *
     * @var array<string, array<string, mixed>>
     */
    protected static $_columnExtras = ['string' => ['collate' => null], 'char' => ['collate' => null], 'text' => ['collate' => null], 'tinyinteger' => ['unsigned' => null], 'smallinteger' => ['unsigned' => null], 'integer' => ['unsigned' => null, 'autoIncrement' => null], 'biginteger' => ['unsigned' => null, 'autoIncrement' => null], 'decimal' => ['unsigned' => null], 'float' => ['unsigned' => null]];
    /**
     ...
     * @var array<string, mixed>
     */
    protected static $_indexKeys = ['type' => null, 'columns' => [], 'length' => [], 'references' => [], 'update' => 'restrict', 'delete' => 'restrict'];
    /**
     * Names of the valid index types.
     *
     * @var array<string>
     */
    protected static $_validIndexTypes = [self::INDEX_INDEX, self::INDEX_FULLTEXT];
    /**
     * Names of the valid constraint types.
     *
     * @var array<string>
     */
    protected static $_validConstraintTypes = [self::CONSTRAINT_PRIMARY, self::CONSTRAINT_UNIQUE, self::CONSTRAINT_FOREIGN];
    /**
     * Names of the valid foreign key actions.
     *
     * @var array<string>
     */
    protected static $_validForeignKeyActions = [self::ACTION_CASCADE, self::ACTION_SET_NULL, self::ACTION_SET_DEFAULT, self::ACTION_NO_ACTION, self::ACTION_RESTRICT];
    /**
    ...
    public function isNullable(string $name): bool
    {
        ...
        return $this->_columns[$name]['null'] === \true;
    }
    /**
     * @inheritDoc
     */
    public function defaultValues(): array
    {
        ...
        foreach ($this->_columns as $name => $data) {
            ...
            if ($data['default'] === null && $data['null'] !== \true) {
                continue;
            }
    ...
    public function addIndex(string $name, $attrs)
    {
        ...
        if (!in_array($attrs['type'], static::$_validIndexTypes, \true)) {
            throw new DatabaseException(sprintf('Invalid index type "%s" in index "%s" in table "%s".', $attrs['type'], $name, $this->_table));
        }
    ...
    public function addConstraint(string $name, $attrs)
    {
        ...
        if ($attrs['type'] === static::CONSTRAINT_FOREIGN) {
            ...
            if (isset($this->_constraints[$name])) {
                $this->_constraints[$name]['columns'] = array_unique(array_merge($this->_constraints[$name]['columns'], $attrs['columns']));
                if (isset($this->_constraints[$name]['references'])) {
                    $this->_constraints[$name]['references'][1] = array_unique(array_merge((array) $this->_constraints[$name]['references'][1], [$attrs['references'][1]]));
                }
                return $this;
            }
        } else {
    ...
    protected function _checkForeignKey(array $attrs): array
    {
        ...
        if (!in_array($attrs['update'], static::$_validForeignKeyActions)) {
            throw new DatabaseException(sprintf('Update action is invalid. Must be one of %s', implode(',', static::$_validForeignKeyActions)));
        }
    ...
    {
        return ['table' => $this->_table, 'columns' => $this->_columns, 'indexes' => $this->_indexes, 'constraints' => $this->_constraints, 'options' => $this->_options, 'typeMap' => $this->_typeMap, 'temporary' => $this->_temporary];
    }
}
