...
use MyVendorPrefix\Cake\Database\DriverInterface;
use MyVendorPrefix\Cake\Database\Exception\DatabaseException;
/**
...
class MysqlSchemaDialect extends SchemaDialect
{
    ...
    {
        $schema->setOptions(['engine' => $row['Engine'], 'collation' => $row['Collation']]);
    }
    /**
    ...
    public function convertColumnDescription(TableSchema $schema, array $row): void
    {
        ...
        $field += ['null' => $row['Null'] === 'YES', 'default' => $row['Default'], 'collate' => $row['Collation'], 'comment' => $row['Comment']];
    ...
    public function convertIndexDescription(TableSchema $schema, array $row): void
    {
        ...
        if ($isIndex) {
            $schema->addIndex($name, ['type' => $type, 'columns' => $columns, 'length' => $length]);
        } else {
            $schema->addConstraint($name, ['type' => $type, 'columns' => $columns, 'length' => $length]);
        }
    }
    /**
     * @inheritDoc
    ...
    public function columnSql(TableSchema $schema, string $name): string
    {
        ...
        $hasLength = [TableSchema::TYPE_INTEGER, TableSchema::TYPE_CHAR, TableSchema::TYPE_SMALLINTEGER, TableSchema::TYPE_TINYINTEGER, TableSchema::TYPE_STRING];
        if (in_array($data['type'], $hasLength, \true) && isset($data['length'])) {
            $out .= '(' . $data['length'] . ')';
        }
        $lengthAndPrecisionTypes = [TableSchema::TYPE_FLOAT, TableSchema::TYPE_DECIMAL];
        if (in_array($data['type'], $lengthAndPrecisionTypes, \true) && isset($data['length'])) {
        ...
        $hasUnsigned = [TableSchema::TYPE_TINYINTEGER, TableSchema::TYPE_SMALLINTEGER, TableSchema::TYPE_INTEGER, TableSchema::TYPE_BIGINTEGER, TableSchema::TYPE_FLOAT, TableSchema::TYPE_DECIMAL];
        if (in_array($data['type'], $hasUnsigned, \true) && isset($data['unsigned']) && $data['unsigned'] === \true) {
            $out .= ' UNSIGNED';
        }
        $hasCollate = [TableSchema::TYPE_TEXT, TableSchema::TYPE_CHAR, TableSchema::TYPE_STRING];
        ...
        $addAutoIncrement = $schema->getPrimaryKey() === [$name] && !$schema->hasAutoincrement() && !isset($data['autoIncrement']);
        if (in_array($data['type'], [TableSchema::TYPE_INTEGER, TableSchema::TYPE_BIGINTEGER]) && ($data['autoIncrement'] === \true || $addAutoIncrement)) {
            $out .= ' AUTO_INCREMENT';
        }
        $timestampTypes = [TableSchema::TYPE_TIMESTAMP, TableSchema::TYPE_TIMESTAMP_FRACTIONAL, TableSchema::TYPE_TIMESTAMP_TIMEZONE];
        if (isset($data['null']) && $data['null'] === \true && in_array($data['type'], $timestampTypes, \true)) {
            $out .= ' NULL';
            unset($data['default']);
        }
        $dateTimeTypes = [TableSchema::TYPE_DATETIME, TableSchema::TYPE_DATETIME_FRACTIONAL, TableSchema::TYPE_TIMESTAMP, TableSchema::TYPE_TIMESTAMP_FRACTIONAL, TableSchema::TYPE_TIMESTAMP_TIMEZONE];
        if (isset($data['default']) && in_array($data['type'], $dateTimeTypes) && strpos(strtolower($data['default']), 'current_timestamp') !== \false) {
...
// phpcs:disable
class_alias('MyVendorPrefix\Cake\Database\Schema\MysqlSchemaDialect', 'MyVendorPrefix\Cake\Database\Schema\MysqlSchema');
// phpcs:enable
