 ...
 */
namespace MyVendorPrefix\Cake\Database\Schema;
...
class SqlserverSchemaDialect extends SchemaDialect
{
    ...
    protected function _convertColumn(string $col, ?int $length = null, ?int $precision = null, ?int $scale = null): array
    {
        ...
        if (strpos($col, 'numeric') !== \false || strpos($col, 'money') !== \false || strpos($col, 'decimal') !== \false) {
    ...
    {
        $field = $this->_convertColumn($row['type'], $row['char_length'] !== null ? (int) $row['char_length'] : null, $row['precision'] !== null ? (int) $row['precision'] : null, $row['scale'] !== null ? (int) $row['scale'] : null);
        if (!empty($row['autoincrement'])) {
            $field['autoIncrement'] = \true;
        }
        $field += ['null' => $row['null'] === '1', 'default' => $this->_defaultValue($field['type'], $row['default']), 'collate' => $row['collation_name']];
    ...
    public function convertIndexDescription(TableSchema $schema, array $row): void
    {
        ...
        if ($type === TableSchema::CONSTRAINT_PRIMARY || $type === TableSchema::CONSTRAINT_UNIQUE) {
            $schema->addConstraint($name, ['type' => $type, 'columns' => $columns]);
            return;
        }
        $schema->addIndex($name, ['type' => $type, 'columns' => $columns]);
    }
    /**
     * @inheritDoc
     */
    ...
    {
        $data = ['type' => TableSchema::CONSTRAINT_FOREIGN, 'columns' => [$row['column']], 'references' => [$row['reference_table'], $row['reference_column']], 'update' => $this->_convertOnClause($row['update_type']), 'delete' => $this->_convertOnClause($row['delete_type'])];
    ...
    public function columnSql(TableSchema $schema, string $name): string
    {
        ...
        if ($data['type'] === TableSchema::TYPE_BINARY) {
            if (!isset($data['length']) || in_array($data['length'], [TableSchema::LENGTH_MEDIUM, TableSchema::LENGTH_LONG], \true)) {
                $data['length'] = 'MAX';
            }
            if ($data['length'] === 1) {
                $out .= ' BINARY(1)';
        ...
        if ($data['type'] === TableSchema::TYPE_STRING || $data['type'] === TableSchema::TYPE_TEXT && $data['length'] === TableSchema::LENGTH_TINY) {
            $type = ' NVARCHAR';
        ...
        if (in_array($data['type'], $hasCollate, \true) && isset($data['collate']) && $data['collate'] !== '') {
            $out .= ' COLLATE ' . $data['collate'];
        }
        $precisionTypes = [TableSchema::TYPE_FLOAT, TableSchema::TYPE_DATETIME, TableSchema::TYPE_DATETIME_FRACTIONAL, TableSchema::TYPE_TIMESTAMP, TableSchema::TYPE_TIMESTAMP_FRACTIONAL];
        if (in_array($data['type'], $precisionTypes, \true) && isset($data['precision'])) {
            $out .= '(' . (int) $data['precision'] . ')';
        }
        if ($data['type'] === TableSchema::TYPE_DECIMAL && (isset($data['length']) || isset($data['precision']))) {
            $out .= '(' . (int) $data['length'] . ',' . (int) $data['precision'] . ')';
        }
        if (isset($data['null']) && $data['null'] === \false) {
            $out .= ' NOT NULL';
        }
        $dateTimeTypes = [TableSchema::TYPE_DATETIME, TableSchema::TYPE_DATETIME_FRACTIONAL, TableSchema::TYPE_TIMESTAMP, TableSchema::TYPE_TIMESTAMP_FRACTIONAL];
        $dateTimeDefaults = ['current_timestamp', 'getdate()', 'getutcdate()', 'sysdatetime()', 'sysutcdatetime()', 'sysdatetimeoffset()'];
        if (isset($data['default']) && in_array($data['type'], $dateTimeTypes, \true) && in_array(strtolower($data['default']), $dateTimeDefaults, \true)) {
    ...
    public function indexSql(TableSchema $schema, string $name): string
    {
        ...
        return sprintf('CREATE INDEX %s ON %s (%s)', $this->_driver->quoteIdentifier($name), $this->_driver->quoteIdentifier($schema->name()), implode(', ', $columns));
    }
    /**
     * @inheritDoc
     */
    ...
    protected function _keySql(string $prefix, array $data): string
    {
        ...
        if ($data['type'] === TableSchema::CONSTRAINT_FOREIGN) {
            return $prefix . sprintf(' FOREIGN KEY (%s) REFERENCES %s (%s) ON UPDATE %s ON DELETE %s', implode(', ', $columns), $this->_driver->quoteIdentifier($data['references'][0]), $this->_convertConstraintColumns($data['references'][1]), $this->_foreignOnClause($data['update']), $this->_foreignOnClause($data['delete']));
        }
...
// phpcs:disable
class_alias('MyVendorPrefix\Cake\Database\Schema\SqlserverSchemaDialect', 'MyVendorPrefix\Cake\Database\Schema\SqlserverSchema');
// phpcs:enable
