...
class PostgresSchemaDialect extends SchemaDialect
{
    ...
    protected function _convertColumn(string $column): array
    {
        ...
        if (in_array($col, ['date', 'time', 'boolean'], \true)) {
            return ['type' => $col, 'length' => null];
        }
        if (in_array($col, ['timestamptz', 'timestamp with time zone'], \true)) {
    ...
    public function convertColumnDescription(TableSchema $schema, array $row): void
    {
        ...
        $field += ['default' => $this->_defaultValue($row['default']), 'null' => $row['null'] === 'YES', 'collate' => $row['collation_name'], 'comment' => $row['comment']];
    ...
    public function columnSql(TableSchema $schema, string $name): string
    {
        ...
        $hasPrecision = [TableSchema::TYPE_FLOAT, TableSchema::TYPE_DATETIME, TableSchema::TYPE_DATETIME_FRACTIONAL, TableSchema::TYPE_TIMESTAMP, TableSchema::TYPE_TIMESTAMP_FRACTIONAL, TableSchema::TYPE_TIMESTAMP_TIMEZONE];
        ...
        if (isset($data['default']) && in_array($data['type'], $datetimeTypes) && strtolower($data['default']) === 'current_timestamp') {
    ...
    {
        $sql = sprintf('DROP TABLE %s CASCADE', $this->_driver->quoteIdentifier($schema->name()));
        return [$sql];
    }
}
// phpcs:disable
class_alias('MyVendorPrefix\Cake\Database\Schema\PostgresSchemaDialect', 'MyVendorPrefix\Cake\Database\Schema\PostgresSchema');
// phpcs:enable
