...
class SqliteSchemaDialect extends SchemaDialect
{
    ...
    {
        $sql = sprintf('PRAGMA table_info(%s)', $this->_driver->quoteIdentifier($tableName));
        return [$sql, []];
    }
    /**
     * @inheritDoc
    ...
    {
        $sql = sprintf('PRAGMA index_list(%s)', $this->_driver->quoteIdentifier($tableName));
        return [$sql, []];
    }
    /**
     * {@inheritDoc}
    ...
    public function columnSql(TableSchema $schema, string $name): string
    {
        ...
        $integerTypes = [TableSchema::TYPE_TINYINTEGER, TableSchema::TYPE_SMALLINTEGER, TableSchema::TYPE_INTEGER];
        if (in_array($data['type'], $integerTypes, \true) && isset($data['length']) && $schema->getPrimaryKey() !== [$name]) {
            $out .= '(' . (int) $data['length'] . ')';
        }
        $hasPrecision = [TableSchema::TYPE_FLOAT, TableSchema::TYPE_DECIMAL];
        if (in_array($data['type'], $hasPrecision, \true) && (isset($data['length']) || isset($data['precision']))) {
        ...
        $timestampTypes = [TableSchema::TYPE_DATETIME, TableSchema::TYPE_DATETIME_FRACTIONAL, TableSchema::TYPE_TIMESTAMP, TableSchema::TYPE_TIMESTAMP_FRACTIONAL, TableSchema::TYPE_TIMESTAMP_TIMEZONE];
    ...
    public function constraintSql(TableSchema $schema, string $name): string
    {
        ...
        if ($data['type'] === TableSchema::CONSTRAINT_PRIMARY && count($data['columns']) === 1 && $schema->getColumn($data['columns'][0])['type'] === TableSchema::TYPE_INTEGER) {
            return '';
        }
        $clause = '';
        $type = '';
...
// phpcs:disable
class_alias('MyVendorPrefix\Cake\Database\Schema\SqliteSchemaDialect', 'MyVendorPrefix\Cake\Database\Schema\SqliteSchema');
// phpcs:enable
