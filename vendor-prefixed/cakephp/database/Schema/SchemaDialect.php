...
use MyVendorPrefix\Cake\Database\Type\ColumnSchemaAwareInterface;
...
abstract class SchemaDialect
{
     ...
     *  custom column SQL.
     */
    protected function _getTypeSpecificColumnSql(string $columnType, TableSchemaInterface $schema, string $column): ?string
    {
    ...
    {
        $sql = sprintf('DROP TABLE %s', $this->_driver->quoteIdentifier($schema->name()));
        return [$sql];
    }
    /**
...
// phpcs:disable
class_alias('MyVendorPrefix\Cake\Database\Schema\SchemaDialect', 'MyVendorPrefix\Cake\Database\Schema\BaseSchema');
// phpcs:enable
