...
use ArrayIterator;
use MyVendorPrefix\Cake\Event\EventDispatcherInterface;
use MyVendorPrefix\Cake\Event\EventListenerInterface;
use Countable;
    ...
    protected function _checkDuplicate(string $name, array $config): void
    {
        ...
        foreach ($config as $key => $value) {
            ...
            if (isset($existingConfig[$key]) && $existingConfig[$key] !== $value) {
                $failure = sprintf(' The `%s` key has a value of `%s` but previously had a value of `%s`', $key, json_encode($value), json_encode($existingConfig[$key]));
                break;
            }
        }
        if ($failure) {
