 ...
 */
namespace MyVendorPrefix\Cake\Core\Exception;
...
class CakeException extends RuntimeException
{
    ...
    public function responseHeader($header = null, $value = null): ?array
    {
        ...
        deprecationWarning('Setting HTTP response headers from Exception directly is deprecated. ' . 'If your exceptions extend Exception, they must now extend HttpException. ' . 'You should only set HTTP headers on HttpException instances via the `setHeaders()` method.');
        if (is_array($header)) {
...
// phpcs:disable
class_alias('MyVendorPrefix\Cake\Core\Exception\CakeException', 'MyVendorPrefix\Cake\Core\Exception\Exception');
// phpcs:enable
