...
use MyVendorPrefix\Cake\Utility\Crypto\OpenSsl;
...
class Security
{
    ...
    public static function hash(string $string, ?string $algorithm = null, $salt = \false): string
    {
        ...
        if (!in_array($algorithm, $availableAlgorithms, \true)) {
            throw new RuntimeException(sprintf('The hash type `%s` was not found. Available algorithms are: %s', $algorithm, implode(', ', $availableAlgorithms)));
        }
        if ($salt) {
            if (!is_string($salt)) {
                $salt = static::getSalt();
