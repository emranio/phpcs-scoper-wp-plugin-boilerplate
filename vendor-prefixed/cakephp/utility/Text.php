...
use function MyVendorPrefix\Cake\I18n\__d;
/**
...
class Text
{
    ...
    {
        $defaults = ['before' => ':', 'after' => '', 'escape' => '\\', 'format' => null, 'clean' => \false];
        $options += $defaults;
        if (empty($data)) {
            return $options['clean'] ? static::cleanInsert($str, $options) : $str;
        }
        if (strpos($str, '?') !== \false && is_numeric(key($data))) {
            deprecationWarning('Using Text::insert() with `?` placeholders is deprecated. ' . 'Use sprintf() with `%s` placeholders instead.');
            $offset = 0;
    ...
    public static function highlight(string $text, $phrase, array $options = []): string
    {
        ...
        $defaults = ['format' => '<span class="highlight">\1</span>', 'html' => \false, 'regex' => '|%s|iu', 'limit' => -1];
        $options += $defaults;
        if (is_array($phrase)) {
            $replace = [];
            $with = [];
        ...
        return preg_replace(sprintf($options['regex'], $phrase), $options['format'], $text, $options['limit']);
    }
    /**
    ...
    protected static function _strlen(string $text, array $options): int
    {
        ...
        $replace = preg_replace_callback($pattern, function ($match) use ($strlen) {
