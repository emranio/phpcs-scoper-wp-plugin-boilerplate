 ...
 */
namespace MyVendorPrefix\Phinx\Console\Command;
...
use MyVendorPrefix\Symfony\Component\Console\Question\ChoiceQuestion;
use MyVendorPrefix\Symfony\Component\Console\Question\ConfirmationQuestion;
...
class Create extends AbstractCommand
{
     ...
     */
    public const CREATION_INTERFACE = 'MyVendorPrefix\Phinx\Migration\CreationInterface';
    /**
     * {@inheritDoc}
     *
     * @return void
    ...
    {
        parent::configure();
        $this->setDescription('Create a new migration')->addArgument('name', InputArgument::OPTIONAL, 'Class name of the migration (in CamelCase)')->setHelp(sprintf('%sCreates a new database migration%s', \PHP_EOL, \PHP_EOL));
    ...
    {
        return new ConfirmationQuestion('Create migrations directory? [y]/n ', \true);
    }
    /**
    ...
    protected function getMigrationPath(InputInterface $input, OutputInterface $output)
    {
        ...
        if (empty($paths)) {
            throw new Exception('You probably used curly braces to define migration path in your Phinx configuration file, ' . 'but no directories have been matched using this pattern. ' . 'You need to create a migration directory manually.');
        }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $classes = ['$namespaceDefinition' => $namespace !== null ? \PHP_EOL . 'namespace ' . $namespace . ';' . \PHP_EOL : '', '$namespace' => $namespace, '$useClassName' => $this->getConfig()->getMigrationBaseClassName(\false), '$className' => $className, '$version' => Util::getVersionFromFileName($fileName), '$baseClassName' => $this->getConfig()->getMigrationBaseClassName(\true)];
