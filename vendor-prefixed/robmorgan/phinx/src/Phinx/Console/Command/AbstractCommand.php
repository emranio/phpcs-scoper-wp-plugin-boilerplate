...
use MyVendorPrefix\Phinx\Migration\Manager;
use MyVendorPrefix\Phinx\Util\Util;
use MyVendorPrefix\Symfony\Component\Config\FileLocator;
...
abstract class AbstractCommand extends Command
{
    ...
    protected function locateConfigFile(InputInterface $input)
    {
        ...
        if (!$useDefault) {
            ...
            return $locator->locate($configFile, $cwd, \true);
        }
    ...
    {
        if (!is_dir($path)) {
            throw new InvalidArgumentException(sprintf('Seed directory "%s" does not exist', $path));
        }
        if (!is_writable($path)) {
            throw new InvalidArgumentException(sprintf('Seed directory "%s" is not writable', $path));
        }
    }
    /**
