...
class SeedCreate extends AbstractCommand
{
    ...
    {
        parent::configure();
        $this->setDescription('Create a new database seeder')->addArgument('name', InputArgument::REQUIRED, 'What is the name of the seeder?')->addOption('path', null, InputOption::VALUE_REQUIRED, 'Specify the path in which to create this seeder')->setHelp(sprintf('%sCreates a new database seeder%s', \PHP_EOL, \PHP_EOL));
    ...
    {
        return new ConfirmationQuestion('Create seeds directory? [y]/n ', \true);
    }
    /**
    ...
    protected function getSeedPath(InputInterface $input, OutputInterface $output)
    {
        ...
        if (empty($paths)) {
            throw new Exception('You probably used curly braces to define seed path in your Phinx configuration file, ' . 'but no directories have been matched using this pattern. ' . 'You need to create a seed directory manually.');
        }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if ($altTemplate && !is_file($altTemplate)) {
            throw new InvalidArgumentException(sprintf('The template file "%s" does not exist', $altTemplate));
        }
        ...
        $classes = ['$namespaceDefinition' => $namespace !== null ? 'namespace ' . $namespace . ';' : '', '$namespace' => $namespace, '$useClassName' => $config->getSeedBaseClassName(\false), '$className' => $className, '$baseClassName' => $config->getSeedBaseClassName(\true)];
