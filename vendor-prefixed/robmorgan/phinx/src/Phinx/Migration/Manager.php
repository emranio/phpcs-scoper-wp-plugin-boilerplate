 ...
 */
namespace MyVendorPrefix\Phinx\Migration;

use DateTime;
use InvalidArgumentException;
use MyVendorPrefix\Phinx\Config\Config;
use MyVendorPrefix\Phinx\Config\ConfigInterface;
use MyVendorPrefix\Phinx\Config\NamespaceAwareInterface;
use MyVendorPrefix\Phinx\Console\Command\AbstractCommand;
use MyVendorPrefix\Phinx\Migration\Manager\Environment;
use MyVendorPrefix\Phinx\Seed\AbstractSeed;
use MyVendorPrefix\Phinx\Seed\SeedInterface;
use MyVendorPrefix\Phinx\Util\Util;
use MyVendorPrefix\Psr\Container\ContainerInterface;
...
class Manager
{
            ...
            foreach ($sortedMigrations as $migration) {
                ...
                if ($version) {
                    ...
                    foreach ($missingVersions as $missingVersionCreationTime => $missingVersion) {
                        ...
                        } else if ($missingVersion['start_time'] > $version['start_time']) {
                            break;
                        } elseif ($missingVersion['start_time'] == $version['start_time'] && $missingVersion['version'] > $version['version']) {
                            break;
                        }
                        $this->printMissingVersion($missingVersion, $maxNameLength);
                        unset($missingVersions[$missingVersionCreationTime]);
                    ...
                    $hasDownMigration = \true;
                    $status = '   <error>down</error> ';
                }
                $maxNameLength = max($maxNameLength, strlen($migration->getName()));
                $output->writeln(sprintf('%s %14.0f  %19s  %19s  <comment>%s</comment>', $status, $migration->getVersion(), $version ? $version['start_time'] : '', $version ? $version['end_time'] : '', $migration->getName()), $this->verbosityLevel);
        ...
        if ($format !== null) {
            switch ($format) {
                case AbstractCommand::FORMAT_JSON:
                    ...
                    $output->writeln(json_encode(['pending_count' => $pendingMigrationCount, 'missing_count' => $missingCount, 'total_count' => $migrationCount + $missingCount, 'migrations' => $finalMigrations]));
                    break;
                default:
                    $output->writeln('<info>Unsupported format: ' . $format . '</info>');
            }
        }
        return ['hasMissingMigration' => $hasMissingMigration, 'hasDownMigration' => $hasDownMigration];
    }
    /**
    ...
    {
        $this->getOutput()->writeln(sprintf('     <error>up</error>  %14.0f  %19s  %19s  <comment>%s</comment>  <error>** MISSING MIGRATION FILE **</error>', $version['version'], $version['start_time'], $version['end_time'], str_pad($version['migration_name'], $maxNameLength, ' ')));
        if ($version && $version['breakpoint']) {
    ...
    public function migrate($environment, $version = null, $fake = \false)
    {
        ...
        } else if ($version != 0 && !isset($migrations[$version])) {
    ...
    public function executeMigration($name, MigrationInterface $migration, $direction = MigrationInterface::UP, $fake = \false)
    {
        ...
        $this->getOutput()->writeln(' ==' . ' <info>' . $migration->getVersion() . ' ' . $migration->getName() . ':</info>' . ' <comment>' . ($direction === MigrationInterface::UP ? 'migrating' : 'reverting') . '</comment>', $this->verbosityLevel);
        // Execute the migration and log the time elapsed.
        $start = microtime(\true);
        $this->getEnvironment($name)->executeMigration($migration, $direction, $fake);
        $end = microtime(\true);
        $this->getOutput()->writeln(' ==' . ' <info>' . $migration->getVersion() . ' ' . $migration->getName() . ':</info>' . ' <comment>' . ($direction === MigrationInterface::UP ? 'migrated' : 'reverted') . ' ' . sprintf('%.4fs', $end - $start) . '</comment>', $this->verbosityLevel);
    }
    /**
    ...
    {
        $this->getOutput()->writeln('', $this->verbosityLevel);
        $this->getOutput()->writeln(' ==' . ' <info>' . $seed->getName() . ':</info>' . ' <comment>seeding</comment>', $this->verbosityLevel);
        // Execute the seeder and log the time elapsed.
        $start = microtime(\true);
        $this->getEnvironment($name)->executeSeed($seed);
        $end = microtime(\true);
        $this->getOutput()->writeln(' ==' . ' <info>' . $seed->getName() . ':</info>' . ' <comment>seeded' . ' ' . sprintf('%.4fs', $end - $start) . '</comment>', $this->verbosityLevel);
    }
    /**
     ...
     * @return void
     */
    public function rollback($environment, $target = null, $force = \false, $targetMustMatchVersion = \true, $fake = \false)
    {
        ...
        foreach ($sortedMigrations as $migration) {
            ...
            if (in_array($migration->getVersion(), $executedVersionCreationTimes)) {
                ...
                if (!$targetMustMatchVersion) {
                    if ($this->getConfig()->isVersionOrderCreationTime() && $executedVersion['version'] <= $target || !$this->getConfig()->isVersionOrderCreationTime() && $executedVersion['start_time'] <= $target) {
                        break;
                    }
                }
    ...
    public function getMigrations($environment)
    {
        if ($this->migrations === null) {
            ...
            if ($this->getOutput()->getVerbosity() >= OutputInterface::VERBOSITY_DEBUG) {
                ...
                $this->getOutput()->writeln(array_map(function ($phpFile) {
            ...
            foreach ($phpFiles as $filePath) {
                ...
                } else if ($this->getOutput()->getVerbosity() >= OutputInterface::VERBOSITY_DEBUG) {
    ...
    protected function markBreakpoint($environment, $version, $mark)
    {
        ...
        $versions = $env->getVersionLog();
        $this->getOutput()->writeln(' Breakpoint ' . ($versions[$version]['breakpoint'] ? 'set' : 'cleared') . ' for <info>' . $version . '</info>' . ' <comment>' . $migrations[$version]->getName() . '</comment>');
    }
    /**
    ...
    {
        $this->getOutput()->writeln(sprintf(' %d breakpoints cleared.', $this->getEnvironment($environment)->getAdapter()->resetAllBreakpoints()));
    }
    /**
