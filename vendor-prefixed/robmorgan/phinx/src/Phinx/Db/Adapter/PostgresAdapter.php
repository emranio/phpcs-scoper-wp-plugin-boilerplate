...
use MyVendorPrefix\Cake\Database\Connection;
use MyVendorPrefix\Cake\Database\Driver\Postgres as PostgresDriver;
use InvalidArgumentException;
use PDO;
use PDOException;
use MyVendorPrefix\Phinx\Db\Table\Column;
use MyVendorPrefix\Phinx\Db\Table\ForeignKey;
use MyVendorPrefix\Phinx\Db\Table\Index;
use MyVendorPrefix\Phinx\Db\Table\Table;
use MyVendorPrefix\Phinx\Db\Util\AlterInstructions;
use MyVendorPrefix\Phinx\Util\Literal;
...
class PostgresAdapter extends PdoAdapter
{
     ...
     * @var string[]
     */
    protected static $specificColumnTypes = [self::PHINX_TYPE_JSON, self::PHINX_TYPE_JSONB, self::PHINX_TYPE_CIDR, self::PHINX_TYPE_INET, self::PHINX_TYPE_MACADDR, self::PHINX_TYPE_INTERVAL, self::PHINX_TYPE_BINARYUUID];
    ...
    public function connect()
    {
        if ($this->connection === null) {
            if (!class_exists('PDO') || !in_array('pgsql', PDO::getAvailableDrivers(), \true)) {
            ...
            try {
                ...
                throw new InvalidArgumentException(sprintf('Schema does not exists: %s', $options['schema']), $exception->getCode(), $exception);
            }
    ...
    public function hasTable($tableName)
    {
        ...
        $parts = $this->getSchemaName($tableName);
        $result = $this->getConnection()->query(sprintf('SELECT *
                FROM information_schema.tables
                WHERE table_schema = %s
                AND table_name = %s', $this->getConnection()->quote($parts['schema']), $this->getConnection()->quote($parts['table'])));
    ...
    public function createTable(Table $table, array $columns = [], array $indexes = [])
    {
        ...
        if (isset($options['comment'])) {
            $queries[] = sprintf('COMMENT ON TABLE %s IS %s', $this->quoteTableName($table->getName()), $this->getConnection()->quote($options['comment']));
        }
    ...
    protected function getChangePrimaryKeyInstructions(Table $table, $newColumns)
    {
        ...
        if (!empty($newColumns)) {
            $sql = sprintf('ADD CONSTRAINT %s PRIMARY KEY (', $this->quoteColumnName($parts['table'] . '_pkey'));
    ...
    protected function getChangeCommentInstructions(Table $table, $newComment)
    {
        ...
        $newComment = $newComment !== null ? $this->getConnection()->quote($newComment) : 'NULL';
        $sql = sprintf('COMMENT ON TABLE %s IS %s', $this->quoteTableName($table->getName()), $newComment);
    ...
    protected function getRenameTableInstructions($tableName, $newTableName)
    {
        ...
        $sql = sprintf('ALTER TABLE %s RENAME TO %s', $this->quoteTableName($tableName), $this->quoteColumnName($newTableName));
    ...
    {
        $sql = sprintf('TRUNCATE TABLE %s RESTART IDENTITY', $this->quoteTableName($tableName));
        $this->execute($sql);
    }
    /**
     * @inheritDoc
    ...
    public function getColumns($tableName)
    {
        $parts = $this->getSchemaName($tableName);
        $columns = [];
        $sql = sprintf('SELECT column_name, data_type, udt_name, is_identity, is_nullable,
             column_default, character_maximum_length, numeric_precision, numeric_scale,
             datetime_precision
             FROM information_schema.columns
             WHERE table_schema = %s AND table_name = %s
             ORDER BY ordinal_position', $this->getConnection()->quote($parts['schema']), $this->getConnection()->quote($parts['table']));
        ...
        foreach ($columnsInfo as $columnInfo) {
            ...
            $column = new Column();
            $column->setName($columnInfo['column_name'])->setType($columnType)->setNull($columnInfo['is_nullable'] === 'YES')->setDefault($columnDefault)->setIdentity($columnInfo['is_identity'] === 'YES')->setScale($columnInfo['numeric_scale']);
            if (preg_match('/\bwith time zone$/', $columnInfo['data_type'])) {
                $column->setTimezone(\true);
            }
    ...
    public function hasColumn($tableName, $columnName)
    {
        ...
        $sql = sprintf('SELECT count(*)
            FROM information_schema.columns
            WHERE table_schema = %s AND table_name = %s AND column_name = %s', $this->getConnection()->quote($parts['schema']), $this->getConnection()->quote($parts['table']), $this->getConnection()->quote($columnName));
    ...
    {
        $instructions = new AlterInstructions();
        $instructions->addAlter(sprintf('ADD %s %s', $this->quoteColumnName($column->getName()), $this->getColumnSqlDefinition($column)));
        if ($column->getComment()) {
    ...
    protected function getRenameColumnInstructions($tableName, $columnName, $newColumnName)
    {
        ...
        $instructions = new AlterInstructions();
        $instructions->addPostStep(sprintf('ALTER TABLE %s RENAME COLUMN %s TO %s', $this->quoteTableName($tableName), $this->quoteColumnName($columnName), $this->quoteColumnName($newColumnName)));
        return $instructions;
    }
    /**
     * @inheritDoc
    ...
    protected function getChangeColumnInstructions($tableName, $columnName, Column $newColumn)
    {
        ...
        $sql = sprintf('ALTER COLUMN %s TYPE %s', $quotedColumnName, $this->getColumnSqlDefinition($newColumn));
        if (in_array($newColumn->getType(), ['smallinteger', 'integer', 'biginteger'], \true)) {
            $sql .= sprintf(' USING (%s::bigint)', $quotedColumnName);
        }
        if ($newColumn->getType() === 'uuid') {
            $sql .= sprintf(' USING (%s::uuid)', $quotedColumnName);
        }
        ...
        if ($newColumn->getType() === 'boolean') {
            $sql .= sprintf(' USING (CASE WHEN %s IS NULL THEN NULL WHEN %s::int=0 THEN FALSE ELSE TRUE END)', $quotedColumnName, $quotedColumnName);
        }
        $instructions->addAlter($sql);
        // process null
        $sql = sprintf('ALTER COLUMN %s', $quotedColumnName);
        if ($newColumn->isNull()) {
            $sql .= ' DROP NOT NULL';
        } else {
            $sql .= ' SET NOT NULL';
        ...
        if ($newColumn->getDefault() !== null) {
            $instructions->addAlter(sprintf('ALTER COLUMN %s SET %s', $quotedColumnName, $this->getDefaultValueDefinition($newColumn->getDefault(), $newColumn->getType())));
        } else {
            //drop default
            $instructions->addAlter(sprintf('ALTER COLUMN %s DROP DEFAULT', $quotedColumnName));
        }
        // rename column
        if ($columnName !== $newColumn->getName()) {
            $instructions->addPostStep(sprintf('ALTER TABLE %s RENAME COLUMN %s TO %s', $this->quoteTableName($tableName), $quotedColumnName, $this->quoteColumnName($newColumn->getName())));
        }
    ...
    protected function getIndexes($tableName)
    {
        $parts = $this->getSchemaName($tableName);
        $indexes = [];
        $sql = sprintf("SELECT\n                i.relname AS index_name,\n                a.attname AS column_name\n            FROM\n                pg_class t,\n                pg_class i,\n                pg_index ix,\n                pg_attribute a,\n                pg_namespace nsp\n            WHERE\n                t.oid = ix.indrelid\n                AND i.oid = ix.indexrelid\n                AND a.attrelid = t.oid\n                AND a.attnum = ANY(ix.indkey)\n                AND t.relnamespace = nsp.oid\n                AND nsp.nspname = %s\n                AND t.relkind = 'r'\n                AND t.relname = %s\n            ORDER BY\n                t.relname,\n                i.relname", $this->getConnection()->quote($parts['schema']), $this->getConnection()->quote($parts['table']));
    ...
    protected function getDropIndexByColumnsInstructions($tableName, $columns)
    {
        ...
        foreach ($indexes as $indexName => $index) {
            ...
            if (empty($a)) {
                return new AlterInstructions([], [sprintf('DROP INDEX IF EXISTS %s', '"' . ($parts['schema'] . '".' . $this->quoteColumnName($indexName)))]);
            }
        }
    ...
    {
        $parts = $this->getSchemaName($tableName);
        $sql = sprintf('DROP INDEX IF EXISTS %s', '"' . ($parts['schema'] . '".' . $this->quoteColumnName($indexName)));
        return new AlterInstructions([], [$sql]);
    }
    /**
     * @inheritDoc
    ...
    {
        $parts = $this->getSchemaName($tableName);
        $rows = $this->fetchAll(sprintf("SELECT\n                    tc.constraint_name,\n                    kcu.column_name\n                FROM information_schema.table_constraints AS tc\n                JOIN information_schema.key_column_usage AS kcu\n                    ON tc.constraint_name = kcu.constraint_name\n                WHERE constraint_type = 'PRIMARY KEY'\n                    AND tc.table_schema = %s\n                    AND tc.table_name = %s\n                ORDER BY kcu.position_in_unique_constraint", $this->getConnection()->quote($parts['schema']), $this->getConnection()->quote($parts['table'])));
        $primaryKey = ['columns' => []];
        foreach ($rows as $row) {
    ...
    {
        $alter = sprintf('ADD %s', $this->getForeignKeySqlDefinition($foreignKey, $table->getName()));
        return new AlterInstructions([$alter]);
    }
    /**
     * @inheritDoc
    ...
    {
        $alter = sprintf('DROP CONSTRAINT %s', $this->quoteColumnName($constraint));
        return new AlterInstructions([$alter]);
    }
    /**
     * @inheritDoc
    ...
    protected function getDropForeignKeyByColumnsInstructions($tableName, $columns)
    {
        ...
        $rows = $this->fetchAll(sprintf($sql, $this->getConnection()->quote($parts['schema']), $this->getConnection()->quote($parts['table']), implode(',', $array), implode(',', $array)));
        foreach ($rows as $row) {
    ...
    protected function getColumnSqlDefinition(Column $column)
    {
        ...
        if ($column->isIdentity()) {
            ...
            if ($sqlType['name'] === static::PHINX_TYPE_DECIMAL && ($column->getPrecision() || $column->getScale())) {
                ...
                $buffer[] = sprintf('(%s,%s)', strtoupper($sqlType['type']), $column->getSrid() ?: $sqlType['srid']);
            } elseif (in_array($sqlType['name'], [self::PHINX_TYPE_TIME, self::PHINX_TYPE_TIMESTAMP], \true)) {
            ...
            } elseif (!in_array($column->getType(), [self::PHINX_TYPE_TINY_INTEGER, self::PHINX_TYPE_SMALL_INTEGER, self::PHINX_TYPE_INTEGER, self::PHINX_TYPE_BIG_INTEGER, self::PHINX_TYPE_BOOLEAN, self::PHINX_TYPE_TEXT, self::PHINX_TYPE_BINARY], \true)) {
    ...
    protected function getColumnCommentSqlDefinition(Column $column, $tableName)
    {
        ...
        return sprintf('COMMENT ON COLUMN %s.%s IS %s;', $this->quoteTableName($tableName), $this->quoteColumnName($column->getName()), $comment);
    }
    /**
    ...
    protected function getIndexSqlDefinition(Index $index, $tableName)
    {
        ...
        return sprintf($createIndexSentence, $index->getType() === Index::UNIQUE ? 'UNIQUE' : '', $this->quoteColumnName($indexName), $this->quoteTableName($tableName), implode(',', $columnNames), $includedColumns);
    }
    /**
    ...
    protected function getForeignKeySqlDefinition(ForeignKey $foreignKey, $tableName)
    {
        ...
        $def = ' CONSTRAINT ' . $this->quoteColumnName($constraintName) . ' FOREIGN KEY ("' . implode('", "', $foreignKey->getColumns()) . '")' . " REFERENCES {$this->quoteTableName($foreignKey->getReferencedTable()->getName())} (\"" . implode('", "', $foreignKey->getReferencedColumns()) . '")';
    ...
    public function hasSchema($schemaName)
    {
        $sql = sprintf('SELECT count(*)
             FROM pg_namespace
             WHERE nspname = %s', $this->getConnection()->quote($schemaName));
        $result = $this->fetchRow($sql);
        return $result['count'] > 0;
    }
    /**
    ...
    protected function isArrayType($columnType)
    {
        ...
        $baseType = $matches[1];
        return in_array($baseType, $this->getColumnTypes(), \true);
    }
    /**
     * @param string $tableName Table name
     * @return array
