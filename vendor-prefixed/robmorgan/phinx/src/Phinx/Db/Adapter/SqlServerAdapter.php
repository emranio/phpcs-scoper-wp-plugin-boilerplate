...
use MyVendorPrefix\Cake\Database\Driver\Sqlserver as SqlServerDriver;
...
class SqlServerAdapter extends PdoAdapter
{
     ...
     * @var string[]
     */
    protected static $specificColumnTypes = [self::PHINX_TYPE_FILESTREAM, self::PHINX_TYPE_BINARYUUID];
    /**
     * @var string
     */
     ...
     * @var bool[]
     */
    protected $signedColumnTypes = [self::PHINX_TYPE_INTEGER => \true, self::PHINX_TYPE_BIG_INTEGER => \true, self::PHINX_TYPE_FLOAT => \true, self::PHINX_TYPE_DECIMAL => \true];
    /**
     * {@inheritDoc}
     *
...
SQL;
    ...
    protected function getColumnSqlDefinition(Column $column, $create = \true)
    {
        ...
        if ($column->getType() instanceof Literal) {
            ...
            $noLimits = ['bigint', 'int', 'tinyint', 'smallint'];
    ...
    protected function getIndexSqlDefinition(Index $index, $tableName)
    {
        ...
        return sprintf('CREATE %s INDEX %s ON %s (%s) %s;', $index->getType() === Index::UNIQUE ? 'UNIQUE' : '', $indexName, $this->quoteTableName($tableName), implode(',', $columnNames), $includedColumns);
    }
    /**
