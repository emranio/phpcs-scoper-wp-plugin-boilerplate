...
use MyVendorPrefix\Cake\Database\Driver\Sqlite as SqliteDriver;
...
use MyVendorPrefix\Phinx\Util\Expression;
...
class SQLiteAdapter extends PdoAdapter
{
     ...
     * @var string[]
     */
    protected static $unsupportedColumnTypes = [self::PHINX_TYPE_BIT, self::PHINX_TYPE_CIDR, self::PHINX_TYPE_ENUM, self::PHINX_TYPE_FILESTREAM, self::PHINX_TYPE_GEOMETRY, self::PHINX_TYPE_INET, self::PHINX_TYPE_INTERVAL, self::PHINX_TYPE_LINESTRING, self::PHINX_TYPE_MACADDR, self::PHINX_TYPE_POINT, self::PHINX_TYPE_POLYGON, self::PHINX_TYPE_SET];
    /**
     * @var string[]
     */
    ...
    public function connect()
    {
        if ($this->connection === null) {
            ...
            } else if (!empty($options['memory']) || $options['name'] === static::MEMORY) {
    ...
    public function createTable(Table $table, array $columns = [], array $indexes = [])
    {
        ...
        foreach ($columns as $column) {
            ...
            if (isset($options['primary_key']) && $column->getIdentity()) {
                ...
                //primary id
                $identityColumnIndex = array_search($column->getName(), $options['primary_key'], \true);
...
PCRE_PATTERN;
    ...
    public function getColumns($tableName)
    {
        ...
        foreach ($rows as $columnInfo) {
            ...
            $column->setName($columnInfo['name'])->setNull((int) $columnInfo['notnull'] !== 1)->setDefault($default)->setType($type['name'])->setLimit($type['limit'])->setScale($type['scale'])->setIdentity($columnInfo['name'] === $identity);
            $columns[] = $column;
        }
        return $columns;
    }
    ...
    protected function getAddColumnInstructions(Table $table, Column $column)
    {
        ...
        $instructions->addPostStep(function ($state) use ($tableName, $column) {
            ...
            $sql = preg_replace(sprintf("/(%s(?:\\/\\*.*?\\*\\/|\\([^)]+\\)|'[^']*?'|[^,])+)([,)])/", $this->quoteColumnName($finalColumnName)), sprintf('$1, %s %s$2', $this->quoteColumnName($column->getName()), $this->getColumnSqlDefinition($column)), $state['createSQL'], 1);
            $this->execute($sql);
            return $state;
        });
    ...
    {
        if ($constraint !== null) {
            return preg_match("/,?\\sCONSTRAINT\\s" . preg_quote($this->quoteColumnName($constraint)) . ' FOREIGN KEY/', $this->getDeclaringSql($tableName)) === 1;
        }
        $columns = array_map('strtolower', (array) $columns);
    ...
    protected function getAddForeignKeyInstructions(Table $table, ForeignKey $foreignKey)
    {
        ...
        $instructions->addPostStep(function ($state) use ($foreignKey, $tableName) {
            ...
            foreach (array_keys($indexes) as $indexName) {
                if (strpos($indexName, 'sqlite_autoindex_') !== 0) {
                    $sql .= sprintf('DROP INDEX %s%s; ', $schema, $this->quoteColumnName($indexName));
                    $createIndexSQL = $this->getDeclaringIndexSQL($tableName, $indexName);
                    $sql .= preg_replace("/\\b{$tableName}\\b/", $tmpTableName, $createIndexSQL);
                }
            }
            $this->execute($sql);
            return $state;
    ...
    public function getSqlType($type, $limit = null)
    {
        ...
        } elseif (in_array($typeLC, static::$unsupportedColumnTypes, \true)) {
    ...
    public function getPhinxType($sqlTypeDef)
    {
        ...
        if ($sqlTypeDef === null) {
            ...
            if (in_array($typeLC, ['tinyint', 'tinyinteger'], \true) && $limit === 1) {
    ...
    protected function getColumnSqlDefinition(Column $column)
    {
        ...
        if ($isLiteralType) {
            ...
            $limitable = in_array(strtoupper($sqlType['name']), $this->definitionsWithLimits, \true);
