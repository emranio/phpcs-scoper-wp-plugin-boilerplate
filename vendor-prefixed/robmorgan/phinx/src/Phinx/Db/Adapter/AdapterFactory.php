...
class AdapterFactory
{
     ...
     * @var string[]
     */
    protected $adapters = ['mysql' => 'MyVendorPrefix\Phinx\Db\Adapter\MysqlAdapter', 'pgsql' => 'MyVendorPrefix\Phinx\Db\Adapter\PostgresAdapter', 'sqlite' => 'MyVendorPrefix\Phinx\Db\Adapter\SQLiteAdapter', 'sqlsrv' => 'MyVendorPrefix\Phinx\Db\Adapter\SqlServerAdapter'];
    /**
     * Class map of adapters wrappers, indexed by name.
     *
     * @var string[]
     */
    protected $wrappers = ['prefix' => 'MyVendorPrefix\Phinx\Db\Adapter\TablePrefixAdapter', 'proxy' => 'MyVendorPrefix\Phinx\Db\Adapter\ProxyAdapter', 'timed' => 'MyVendorPrefix\Phinx\Db\Adapter\TimedOutputAdapter'];
    /**
    ...
    {
        if (!is_subclass_of($class, 'MyVendorPrefix\Phinx\Db\Adapter\AdapterInterface')) {
            throw new RuntimeException(sprintf('Adapter class "%s" must implement Phinx\Db\Adapter\AdapterInterface', $class));
        }
    ...
    {
        if (!is_subclass_of($class, 'MyVendorPrefix\Phinx\Db\Adapter\WrapperInterface')) {
            throw new RuntimeException(sprintf('Wrapper class "%s" must be implement Phinx\Db\Adapter\WrapperInterface', $class));
        }
    ...
    protected function getWrapperClass($name)
    {
        if (empty($this->wrappers[$name])) {
            throw new RuntimeException(sprintf('Wrapper "%s" has not been registered', $name));
        }
