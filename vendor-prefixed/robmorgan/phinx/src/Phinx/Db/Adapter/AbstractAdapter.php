...
use MyVendorPrefix\Phinx\Db\Table;
...
use MyVendorPrefix\Symfony\Component\Console\Output\NullOutput;
...
abstract class AbstractAdapter implements AdapterInterface
{
    ...
    public function setDataDomain(array $dataDomain)
    {
        ...
        foreach ($dataDomain as $type => $options) {
            if (!isset($options['type'])) {
                throw new \InvalidArgumentException(sprintf('You must specify a type for data domain type "%s".', $type));
            }
            // Replace type if it's the name of a Phinx constant
            if (defined('static::' . $options['type'])) {
                $options['type'] = constant('static::' . $options['type']);
            }
            if (!in_array($options['type'], $this->getColumnTypes(), \true)) {
                throw new \InvalidArgumentException(sprintf('An invalid column type "%s" was specified for data domain type "%s".', $options['type'], $type));
            }
            ...
            if (isset($options['limit']) && !is_numeric($options['limit'])) {
                if (!defined('static::' . $options['limit'])) {
                    throw new \InvalidArgumentException(sprintf('An invalid limit value "%s" was specified for data domain type "%s".', $options['limit'], $type));
                }
                $options['limit'] = constant('static::' . $options['limit']);
            }
            // Save the data domain types in a more suitable format
            $this->dataDomain[$type] = ['type' => $internal_type, 'options' => $options];
        }
        return $this;
    }
    /**
    ...
    public function createSchemaTable()
    {
        try {
            ...
            $table->addColumn('version', 'biginteger')->addColumn('migration_name', 'string', ['limit' => 100, 'default' => null, 'null' => \true])->addColumn('start_time', 'timestamp', ['default' => null, 'null' => \true])->addColumn('end_time', 'timestamp', ['default' => null, 'null' => \true])->addColumn('breakpoint', 'boolean', ['default' => \false])->save();
        } catch (Exception $exception) {
            throw new InvalidArgumentException('There was a problem creating the schema table: ' . $exception->getMessage(), (int) $exception->getCode(), $exception);
        }
    }
    /**
     * @inheritDoc
    ...
    {
        return $column->getType() instanceof Literal || in_array($column->getType(), $this->getColumnTypes(), \true);
    }
    /**
    ...
    protected function updateCreatedTableName($tableName, $newTableName)
    {
        ...
        $key = array_search($tableName, $this->createdTables, \true);
        if ($key !== \false) {
    ...
    {
        $tableName = $this->quoteTableName($tableName);
        return in_array($tableName, $this->createdTables, \true);
    }
}
