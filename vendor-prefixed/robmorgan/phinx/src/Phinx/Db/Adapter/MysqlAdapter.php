...
use MyVendorPrefix\Cake\Database\Driver\Mysql as MysqlDriver;
...
class MysqlAdapter extends PdoAdapter
{
     ...
     * @var string[]
     */
    protected static $specificColumnTypes = [self::PHINX_TYPE_ENUM, self::PHINX_TYPE_SET, self::PHINX_TYPE_YEAR, self::PHINX_TYPE_JSON, self::PHINX_TYPE_BINARYUUID, self::PHINX_TYPE_TINYBLOB, self::PHINX_TYPE_MEDIUMBLOB, self::PHINX_TYPE_LONGBLOB, self::PHINX_TYPE_MEDIUM_INTEGER];
    /**
     * @var bool[]
     */
    protected $signedColumnTypes = [self::PHINX_TYPE_INTEGER => \true, self::PHINX_TYPE_TINY_INTEGER => \true, self::PHINX_TYPE_SMALL_INTEGER => \true, self::PHINX_TYPE_MEDIUM_INTEGER => \true, self::PHINX_TYPE_BIG_INTEGER => \true, self::PHINX_TYPE_FLOAT => \true, self::PHINX_TYPE_DECIMAL => \true, self::PHINX_TYPE_DOUBLE => \true, self::PHINX_TYPE_BOOLEAN => \true];
    public const TEXT_TINY = 255;
    public const TEXT_SMALL = 255;
    /* deprecated, alias of TEXT_TINY */
    ...
    public const INT_BIG = 1.8446744073709552E+19;
    ...
    public function connect()
    {
        if ($this->connection === null) {
            if (!class_exists('PDO') || !in_array('mysql', PDO::getAvailableDrivers(), \true)) {
    ...
    public function createTable(Table $table, array $columns = [], array $indexes = [])
    {
        ...
        $defaultOptions = ['engine' => 'InnoDB', 'collation' => 'utf8_general_ci'];
        $options = array_merge($defaultOptions, array_intersect_key($this->getOptions(), $defaultOptions), $table->getOptions());
        ...
        if (isset($options['id']) && is_string($options['id'])) {
            ...
            $column = new Column();
            $column->setName($options['id'])->setType('integer')->setSigned($options['signed'] ?? \true)->setIdentity(\true);
    ...
    protected function getRenameTableInstructions($tableName, $newTableName)
    {
        ...
        $sql = sprintf('RENAME TABLE %s TO %s', $this->quoteTableName($tableName), $this->quoteTableName($newTableName));
    ...
    public function getColumns($tableName)
    {
        ...
        foreach ($rows as $columnInfo) {
            ...
            if (is_string($default) && in_array($column->getType(), array_merge(static::PHINX_TYPES_GEOSPATIAL, [static::PHINX_TYPE_BLOB, static::PHINX_TYPE_JSON, static::PHINX_TYPE_TEXT]))) {
    ...
    {
        $alter = sprintf('ADD %s %s', $this->quoteColumnName($column->getName()), $this->getColumnSqlDefinition($column));
    ...
    protected function getRenameColumnInstructions($tableName, $columnName, $newColumnName)
    {
        ...
        foreach ($rows as $row) {
            if (strcasecmp($row['Field'], $columnName) === 0) {
                ...
                $alter = sprintf('CHANGE COLUMN %s %s %s', $this->quoteColumnName($columnName), $this->quoteColumnName($newColumnName), $definition);
                return new AlterInstructions([$alter]);
            }
        }
    ...
    {
        $alter = sprintf('CHANGE %s %s %s%s', $this->quoteColumnName($columnName), $this->quoteColumnName($newColumn->getName()), $this->getColumnSqlDefinition($newColumn), $this->afterClause($newColumn));
        return new AlterInstructions([$alter]);
    }
    /**
     * @inheritDoc
    ...
    protected function getAddIndexInstructions(Table $table, Index $index)
    {
        ...
        if ($index->getType() === Index::FULLTEXT) {
            ...
            $alter = sprintf('ALTER TABLE %s ADD %s', $this->quoteTableName($table->getName()), $this->getIndexSqlDefinition($index));
            $instructions->addPostStep($alter);
        } else {
            $alter = sprintf('ADD %s', $this->getIndexSqlDefinition($index));
    ...
    protected function getDropIndexByColumnsInstructions($tableName, $columns)
    {
        ...
        foreach ($indexes as $indexName => $index) {
            if ($columns == $index['columns']) {
                return new AlterInstructions([sprintf('DROP INDEX %s', $this->quoteColumnName($indexName))]);
            }
        }
    ...
    {
        $alter = sprintf('ADD %s', $this->getForeignKeySqlDefinition($foreignKey));
        return new AlterInstructions([$alter]);
    }
    /**
     * @inheritDoc
    ...
    {
        $alter = sprintf('DROP FOREIGN KEY %s', $constraint);
        return new AlterInstructions([$alter]);
    }
    /**
     * {@inheritDoc}
    ...
    protected function getDropForeignKeyByColumnsInstructions($tableName, $columns)
    {
        $instructions = new AlterInstructions();
        foreach ($columns as $column) {
            $rows = $this->fetchAll(sprintf("SELECT\n                    CONSTRAINT_NAME\n                  FROM information_schema.KEY_COLUMN_USAGE\n                  WHERE REFERENCED_TABLE_SCHEMA = DATABASE()\n                    AND REFERENCED_TABLE_NAME IS NOT NULL\n                    AND TABLE_NAME = '%s'\n                    AND COLUMN_NAME = '%s'\n                  ORDER BY POSITION_IN_UNIQUE_CONSTRAINT", $tableName, $column));
            foreach ($rows as $row) {
        ...
        switch ($type) {
            ...
            case static::PHINX_TYPE_BLOB:
                if ($limit !== null) {
                    ...
                    $sizes = ['tinyblob' => static::BLOB_SMALL, 'blob' => static::BLOB_REGULAR, 'mediumblob' => static::BLOB_MEDIUM];
                    foreach ($sizes as $name => $length) {
                        if ($limit <= $length) {
                            return ['name' => $name];
                        }
            ...
            case static::PHINX_TYPE_INTEGER:
                if ($limit && $limit >= static::INT_TINY) {
                    ...
                    $limits = ['tinyint' => static::INT_DISPLAY_TINY, 'smallint' => static::INT_DISPLAY_SMALL, 'mediumint' => static::INT_DISPLAY_MEDIUM, 'int' => static::INT_DISPLAY_REGULAR, 'bigint' => static::INT_DISPLAY_BIG];
                    foreach ($sizes as $name => $length) {
                        if ($limit >= $length) {
                            $def = ['name' => $name];
    ...
    public function getPhinxType($sqlTypeDef)
    {
        ...
        $phinxType = ['name' => $type, 'limit' => $limit, 'scale' => $scale];
        if ($type === static::PHINX_TYPE_ENUM || $type === static::PHINX_TYPE_SET) {
            ...
            for ($i = 0; $i < $valuesLength; $i++) {
                ...
                if ($char === "'" && !$opened) {
                    $opened = \true;
                } elseif (!$escaped && $i + 1 < $valuesLength && ($char === "'" && $values[$i + 1] === "'" || $char === '\\' && $values[$i + 1] === '\\')) {
                    $escaped = \true;
    ...
    {
        $rows = $this->fetchAll(sprintf('SELECT SCHEMA_NAME FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = \'%s\'', $name));
        foreach ($rows as $row) {
            if (!empty($row)) {
                return \true;
            }
    ...
    protected function getColumnSqlDefinition(Column $column)
    {
        ...
        if (version_compare($this->getAttribute(\PDO::ATTR_SERVER_VERSION), '8', '>=') && in_array($column->getType(), static::PHINX_TYPES_GEOSPATIAL) && !is_null($column->getSrid())) {
        ...
        if (version_compare($this->getAttribute(\PDO::ATTR_SERVER_VERSION), '8', '>=') && is_string($default) && in_array($column->getType(), array_merge(static::PHINX_TYPES_GEOSPATIAL, [static::PHINX_TYPE_BLOB, static::PHINX_TYPE_JSON, static::PHINX_TYPE_TEXT]))) {
    ...
    public function describeTable($tableName)
    {
        $options = $this->getOptions();
        // mysql specific
        $sql = sprintf("SELECT *\n             FROM information_schema.tables\n             WHERE table_schema = '%s'\n             AND table_name = '%s'", $options['name'], $tableName);
