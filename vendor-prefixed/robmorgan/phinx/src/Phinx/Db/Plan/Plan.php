 ...
 */
namespace MyVendorPrefix\Phinx\Db\Plan;
...
use MyVendorPrefix\Phinx\Db\Adapter\AdapterInterface;
use MyVendorPrefix\Phinx\Db\Plan\Solver\ActionSplitter;
...
class Plan
{
    ...
    {
        return [$this->tableUpdates, $this->constraints, $this->indexes, $this->columnRemoves, $this->tableMoves];
    }
    /**
    ...
    {
        return [$this->constraints, $this->tableMoves, $this->indexes, $this->columnRemoves, $this->tableUpdates];
    }
    /**
    ...
    protected function resolveConflicts()
    {
        ...
        foreach ($this->columnRemoves as $columnRemove) {
            foreach ($columnRemove->getActions() as $action) {
                if ($action instanceof RemoveColumn) {
                    [$this->indexes] = $this->forgetDropIndex($action->getTable(), [$action->getColumn()->getName()], $this->indexes);
                }
            }
        }
        ...
        foreach ($this->constraints as $constraint) {
            $constraints = array_merge($constraints, $splitter($this->remapContraintAndIndexConflicts($constraint)));
        }
    ...
    protected function remapContraintAndIndexConflicts(AlterTable $alter)
    {
        ...
        foreach ($alter->getActions() as $action) {
            ...
            if ($action instanceof DropForeignKey) {
                [$this->indexes, $dropIndexActions] = $this->forgetDropIndex($action->getTable(), $action->getForeignKey()->getColumns(), $this->indexes);
    ...
    protected function forgetRemoveColumn(Table $table, array $columns, array $actions)
    {
        ...
        $indexes = array_map(function ($alter) use ($table, $columns, $removeColumnActions) {
            ...
            $newAlter = new AlterTable($table);
            foreach ($alter->getActions() as $action) {
                if ($action instanceof RemoveColumn && in_array($action->getColumn()->getName(), $columns, \true)) {
    ...
    {
        foreach ($actions as $action) {
            if (!$action instanceof AddColumn && !$action instanceof ChangeColumn && !$action instanceof RemoveColumn && !$action instanceof RenameColumn) {
                continue;
