 ...
 */
namespace MyVendorPrefix\Phinx\Db;
...
use MyVendorPrefix\Phinx\Db\Table\Table as TableValue;
...
class Table
{
    ...
    {
        $columns = array_filter($this->getColumns(), function ($column) use ($name) {
    ...
    public function addColumn($columnName, $type = null, $options = [])
    {
        ...
        if (!$this->getAdapter()->isValidColumnType($action->getColumn())) {
            throw new InvalidArgumentException(sprintf('An invalid column type "%s" was specified for column "%s".', $type, $action->getColumn()->getName()));
        }
    ...
    {
        $action = AddForeignKey::build($this->table, $columns, $referencedTable, $referencedColumns, $options, $name);
        $this->actions->addAction($action);
        return $this;
    }
    /**
    ...
    public function addTimestamps($createdAt = 'created_at', $updatedAt = 'updated_at', $withTimezone = \false)
    {
        ...
        if ($createdAt) {
            $this->addColumn($createdAt, 'timestamp', ['default' => 'CURRENT_TIMESTAMP', 'update' => '', 'timezone' => $withTimezone]);
        }
        if ($updatedAt) {
            $this->addColumn($updatedAt, 'timestamp', ['null' => \true, 'default' => null, 'update' => 'CURRENT_TIMESTAMP', 'timezone' => $withTimezone]);
        }
        return $this;
    }
    /**
    ...
    {
        $this->addTimestamps($createdAt, $updatedAt, \true);
        return $this;
    }
    /**
