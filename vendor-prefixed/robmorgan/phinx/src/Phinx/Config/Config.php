 ...
 */
namespace MyVendorPrefix\Phinx\Config;

use Closure;
use InvalidArgumentException;
use MyVendorPrefix\Phinx\Db\Adapter\SQLiteAdapter;
use MyVendorPrefix\Phinx\Util\Util;
use RuntimeException;
use MyVendorPrefix\Symfony\Component\Yaml\Yaml;
...
class Config implements ConfigInterface, NamespaceAwareInterface
{
    ...
    {
        if (!class_exists('MyVendorPrefix\Symfony\Component\Yaml\Yaml', \true)) {
        ...
        if (!is_array($configArray)) {
            throw new RuntimeException(sprintf('File \'%s\' must be valid YAML', $configFilePath));
        }
    ...
    public static function fromJson($configFilePath)
    {
        ...
        $configArray = json_decode(file_get_contents($configFilePath), \true);
        if (!is_array($configArray)) {
            throw new RuntimeException(sprintf('File \'%s\' must be valid JSON', $configFilePath));
        }
        return new static($configArray, $configFilePath);
    }
    /**
    ...
    public function getEnvironment($name)
    {
        ...
        if (isset($environments[$name])) {
            ...
            if (isset($environments[$name]['adapter']) && $environments[$name]['adapter'] === 'sqlite' && !empty($environments[$name]['memory'])) {
    ...
    public function getDefaultEnvironment()
    {
        ...
        if (!empty($env)) {
            ...
            throw new RuntimeException(sprintf('The environment configuration (read from $PHINX_ENVIRONMENT) for \'%s\' is missing', $env));
        }
        ...
        if (isset($this->values['environments']['default_environment'])) {
            ...
            throw new RuntimeException(sprintf('The environment configuration for \'%s\' is missing', $this->values['environments']['default_environment']));
        }
     ...
     * @return string
     */
    public function getMigrationBaseClassName($dropNamespace = \true)
    {
     ...
     * @return string
     */
    public function getSeedBaseClassName($dropNamespace = \true)
    {
