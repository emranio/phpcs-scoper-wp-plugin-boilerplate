...
use MyVendorPrefix\Dbout\WpOrm\Api\WithMetaModelInterface;
use MyVendorPrefix\Dbout\WpOrm\Exceptions\MetaNotSupportedException;
use MyVendorPrefix\Dbout\WpOrm\Exceptions\WpOrmException;
use MyVendorPrefix\Dbout\WpOrm\MetaMappingConfig;
use MyVendorPrefix\Dbout\WpOrm\Orm\AbstractModel;
...
abstract class AbstractWithMetaBuilder extends AbstractBuilder
{
     ...
     */
    protected array $joinCallback = ['inner' => 'join', 'left' => 'leftJoin', 'right' => 'rightJoin'];
    /**
    ...
    {
        $this->joinToMeta($metaKey)->where(sprintf('%s.%s', $metaKey, $this->metaConfig->columnValue), $operator, $value);
        return $this;
    }
    /**
     * @param string $metaKey
    ...
    public function joinToMeta(string $metaKey, string $joinType = 'inner'): self
    {
        ...
        $this->{$join}($joinTable, function ($join) use ($metaKey, $model) {
            ...
            $join->on(sprintf('%s.%s', $metaKey, $this->metaConfig?->columnKey), '=', Database::getInstance()->raw(sprintf("'%s'", $metaKey)))->on(sprintf('%s.%s', $metaKey, $this->metaConfig?->foreignKey), '=', sprintf('%s.%s', $model->getTable(), $model->getKeyName()));
        });
        return $this;
    }
    /**
    ...
    protected function initMeta(): void
    {
        if (!$this->model instanceof WithMetaModelInterface) {
            throw new MetaNotSupportedException(sprintf('Model %s must be implement %s', get_class($this->model), WithMetaModelInterface::class));
        }
        ...
        if (!$reflection->isSubclassOf(AbstractModel::class)) {
            throw new WpOrmException(sprintf('Class %s must extend from %s.', $config->metaClass, AbstractModel::class));
        }
