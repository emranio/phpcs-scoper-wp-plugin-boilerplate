...
use MyVendorPrefix\Dbout\WpOrm\Orm\Query\Grammars\WordPressGrammar;
use MyVendorPrefix\Dbout\WpOrm\Orm\Query\Processors\WordPressProcessor;
use MyVendorPrefix\Dbout\WpOrm\Orm\Schemas\WordPressBuilder;
use MyVendorPrefix\Illuminate\Database\Connection;
use MyVendorPrefix\Illuminate\Database\LostConnectionException;
use MyVendorPrefix\Illuminate\Database\QueryException;
use MyVendorPrefix\Illuminate\Database\Schema\Grammars\Grammar;
use MyVendorPrefix\Illuminate\Database\Schema\Grammars\MySqlGrammar as SchemaGrammar;
/**
...
class Database extends Connection
{
     ...
     * @var \wpdb
     */
    protected \MyVendorPrefix\wpdb $db;
    /**
    ...
    {
        if (!self::$instance instanceof \MyVendorPrefix\Dbout\WpOrm\Orm\Database) {
    ...
    {
        global $wpdb;
        if (!$wpdb instanceof \MyVendorPrefix\wpdb) {
            throw new \Exception('The global variable $wpdb must be instance of \wpdb.');
        }
        $pdo = function (): never {
            throw new \Exception('PDO property can\'t be used.');
        };
        parent::__construct($pdo, defined('DB_NAME') ? DB_NAME : '', $wpdb->prefix, ['name' => 'wp-eloquent-mysql2', 'charset' => $wpdb->charset, 'collate' => $wpdb->collate, 'version' => $wpdb->db_version()]);
        $this->db = $wpdb;
    }
    /**
     * @inheritDoc
     ...
     * @inheritDoc
     */
    public function select($query, $bindings = [], $useReadPdo = \true): array
    {
        return $this->run($query, $bindings, function (string $query, array $bindings) {
            $query = $this->bindParams($query, $bindings);
     ...
     * @inheritDoc
     */
    public function cursor($query, $bindings = [], $useReadPdo = \true): \Generator
    {
    ...
    protected function runQueryCallback($query, $bindings, \Closure $callback): mixed
    {
        try {
            ...
            throw new QueryException($this->getName(), $query, $this->prepareBindings($bindings), $exception);
        }
    }
    /**
     ...
     * @inheritDoc
     */
    public function getSchemaBuilder(): \MyVendorPrefix\Illuminate\Database\Schema\Builder
    {
